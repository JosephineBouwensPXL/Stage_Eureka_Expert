import { transcribeAudioWithElevenLabsStt, transcribeAudioWithLocalStt } from '../../../localSpeechService';
  ...
  async captureOnce({
    ...
    recorder.onstop = async () => {
      try {
        ...
        const transcript = await transcribeAudioWithElevenLabsStt(blob, language);
        if (!transcript.trim()) {
          finalize(null);
          return;
        }
